import React from "react";

const Factor2 = () => {
  return (
    <>
      <div className=" flex border-2 border-[#2b6519]">
        <div className="flex bg-[#2b6519]  w-[140px]  text-white text-center justify-center items-center">
          عامل پنجم:
        </div>
        <div className=" flex bg-[#f1f1f1]  w-[900px] justify-start items-center">
          <p className="mr-2">تجربیات</p>
        </div>
        <div className=" bg-[#f1f1f1] w-[45px] border-r-2 border-[#2b6519]">
          <div className="border-b-2 border-[#2b6519] ">max</div>
          <div>120</div>
        </div>
        <div className=" bg-[#f1f1f1]  w-[45px] border-r-2 border-[#2b6519]">
          <div className="  border-b-2 border-[#2b6519]">js</div>
          <div className="bg-[#2b6519] text-white">100</div>
        </div>
      </div>
      <br />
      <div>
        <p>
          مجموعه معلومات لازم جهت تصدی و انجام وظایف هر شغل که از طریق دوره های
          آموزشی فرد در مدارس و دانشگاه ها بدست می آید.
        </p>
      </div>
      <br />
      <div className="felx mb-1">
        <div className=" bg-[#f6d5b6] ml-[2px] w-[907px] flex  items-center justify-center h-[40px] text-[0.90rem]">
          میزان خسارت و ضایعه وارده احتمالی
        </div>
      </div>
      <div className="flex   ">
        <div className=" bg-[#d7d7d7] ml-[2px] w-[185px] flex  items-center justify-center h-[100px] text-[0.90rem]">
          2 سال تجربه مفید و مرتبط
        </div>
        <div className=" bg-[#d7d7d7] ml-[2px] w-[185px] flex  items-center justify-center h-[100px] text-[0.90rem]">
          4 سال تجربه مفید و مرتبط
        </div>
        <div className=" bg-[#d7d7d7] ml-[2px] w-[185px] flex  items-center justify-center h-[100px] text-[0.90rem]">
          6 سال تجربه مفید و مرتبط
        </div>
        <div className=" bg-[#d7d7d7] ml-[2px] w-[185px] flex  items-center justify-center h-[100px]text-[0.90rem]">
          8 سال تجربه مفید و مرتبط
        </div>
        <div className=" bg-[#d7d7d7] ml-[2px] w-[185px] flex  items-center justify-center h-[100px] text-[0.90rem]">
          10 سال تجربه مفید و مرتبط
        </div>
      </div>
      <div className="flex  ">
        <div className=" bg-[#ecebe0] ml-[2px] w-[185px] mt-[1px] flex  items-center  justify-center">
          25
        </div>
        <div className=" bg-[#ecebe0] ml-[2px] w-[185px] mt-[1px] flex  items-center  justify-center">
          50
        </div>
        <div className=" bg-[#ecebe0] ml-[2px] w-[185px] mt-[1px] flex  items-center  justify-center">
          75
        </div>
        <div className=" bg-[#ecebe0]    ml-[2px] w-[185px] mt-[1px] flex  items-center  justify-center justify-center">
          100
        </div>
        <div className="bg-[#2b6519] text-white  ml-[2px] w-[185px] mt-[1px] flex  items-center  justify-center">
          120
        </div>
      </div>
    </>
  );
};

export default Factor2;
